      ...
      <div className="glass rounded-3xl p-8 max-w-2xl w-full max-h-[90vh] overflow-y-auto space-y-6 animate-in zoom-in duration-300 shadow-2xl border border-white/10">
        <div className="flex items-center justify-between">
          <h2 className="text-2xl font-black text-white flex items-center gap-3">
            <i className="fas fa-layer-group text-accent"></i> {t('levelSelector.selectLevel')}
          </h2>
          <button
              ...
              >
                ...
                <div className="flex flex-col items-center gap-2">
                  ...
                  <div className="text-center">
                    <div className={`text-sm font-black ${isCurrent ? 'text-accent' : isUnlocked ? 'text-white' : 'text-slate-500'
                      }`}>
                      {formatTranslation(t('levelSelector.level'), { level: levelInfo.level })}
                    </div>
                    <div className={`text-[10px] font-bold uppercase tracking-wider mt-1 ${isCurrent ? 'text-accent-soft' : isUnlocked ? 'text-slate-400' : 'text-slate-600'
                      }`}>
                      {levelInfo.persona}
                    </div>
                  </div>
        ...
        {randomModeStats && (
          <div className="glass rounded-2xl p-4 border border-accent-20 bg-accent-5 space-y-2">
            <div className="flex items-center justify-between">
